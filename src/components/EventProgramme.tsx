...
const EventProgramme = () => {
  ...
  const openPrintWindow = (title: string) => {
    if (!programmeRef.current) {
      alert("Programme preview is not ready yet. Please try again.");
      return;
    }
    ...
    if (!printWindow) {
      alert("Please enable pop-ups to download the programme.");
      return;
    }
      ...
      <html>
        <head>
          ...
          <style>
            ...
            body { margin: 0; padding: 24px; background: #012012; }
            #programme-card { margin: 0 auto; }
          </style>
        </head>
        <body>
          ${programmeRef.current.outerHTML}
        </body>
      </html>
    `);
  ...
  const handleDownload = () => openPrintWindow("Event Programme");
  const handlePrint = () => openPrintWindow("Event Programme");
  ...
  return (
    <section className="relative py-20 bg-emerald-dark overflow-hidden floral-pattern">
      ...
      <div ref={programmeRef} id="programme-card" className="container mx-auto px-4">
      ...
      <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4 print:hidden">
        <Button onClick={handleDownload} className="bg-gold text-emerald font-serif gap-2">
          <Download className="w-4 h-4" />
          Download Programme
        </Button>
        <Button onClick={handlePrint} variant="outline" className="border-gold text-gold hover:bg-gold/10 font-serif gap-2">
